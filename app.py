### LICENSE : https://github.com/kamino410/cv-snippets/blob/master/LICENSE
...
class MyCamera():  # Modify this class according to your environment
    ...
    def getGrayscaleFrame(self, index):
        # ret, frame = self.cap.read()
        filename = "./camimg/" + str(index) + ".JPG"
        frame = cv2.cv2.imread(filename)
        ret = True
        if ret:
...
def main():
    ...
    index = 1
    while(True):
        frame = cam.getGrayscaleFrame(index)
        if frame is None:
